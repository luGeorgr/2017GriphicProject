...
int Sphere::setPoint()
{
	...
	{
		roundPoint a = calPoint(0, 0);
		roundPoint b = calPoint(u, vstep);
		roundPoint c = calPoint(u + ustep, vstep);
	...
	for (int i = 1; i<vStepNum - 1; i++)
	{
		...
		{
			roundPoint a = calPoint(u, v);
			roundPoint b = calPoint(u + ustep, v);
			roundPoint c = calPoint(u + ustep, v + vstep);
			roundPoint d = calPoint(u, v + vstep);
	...
	for (int i = 0; i<uStepsNum; i++)
	{
		...
		roundPoint a = calPoint(0, 1);
		roundPoint b = calPoint(u, 1 - vstep);
		roundPoint c = calPoint(u + ustep, 1 - vstep);
		point3.push_back(a.x);
		point3.push_back(a.y);
		point3.push_back(a.z);
		normal3.push_back(a.normalx);
